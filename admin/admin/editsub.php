...
// 设置变量————————————————————————————
    ...
    $result_un = mysqli_query($conn,"SELECT * FROM `admin` WHERE number = '{$number}'");//——————————————————————————————————————连接管理员数据表——————————————————————————————————————————————
    $row_un = mysqli_fetch_assoc($result_un);
    var_dump($row_un['number']);
...
// 判断必填项及数据上传——————————————————————————
        ...
        elseif($row_un['number'] !== null && $onumber !== $number){
            echo "<script type='text/javascript'>;
            alert('管理员学号重复');
            location.href = './edit.php?id=" . $id . "';
            </script>";
        ...
        elseif($uid === ""){
            ...
            $sql = "UPDATE `admin`
                SET name='{$name}',number='{$number}',uid='{$uid}'
                WHERE id='{$id}'";
            $conn->query($sql);
            echo "<script type='text/javascript'>
            alert('编辑成功');
